...
ACTION dgoods::setconfig(symbol_code sym, string version) {
    ...
    // valid symbol
    eosio_assert( sym.is_valid(), "not valid symbol" );
...
ACTION dgoods::create(name issuer,
    ...
    if ( fungible == true ) {
        ...
        eosio_assert(m_supply.amount >= 1, "max_supply for nft must be at least 1");
    }

    // get category_name_id
    config_index config_table( get_self(), get_self().value );
    eosio_assert(config_table.exists(), "Symbol table does not exist, setconfig first");
    ...
    eosio_assert( existing_token == stats_table.end(), "Token with category and token_name exists" );
...
ACTION dgoods::issue(name to,
    ...
    if (dgood_stats.fungible == false) {
        ...
        eosio_assert( q.amount <= (dgood_stats.max_supply.amount - dgood_stats.current_supply), "Cannot issue more than max supply" );
        ...
        eosio_assert( q.precision == dgood_stats.max_supply.precision, string_precision.c_str() );
        add_balance(to, dgood_stats.issuer, category, token_name, dgood_stats.category_name_id, q);
...
ACTION dgoods::burnnft(name owner,
    ...
    for ( auto const& dgood_id: dgood_ids ) {
        ...
        eosio_assert( token.owner == owner, "must be token owner" );
        ...
        eosio_assert( dgood_stats.burnable == true, "Not burnable");
        eosio_assert( dgood_stats.fungible == false, "Cannot call burnnft on fungible token, call burn instead");
...
ACTION dgoods::transfernft(name from,
    ...
    for ( auto const& dgood_id: dgood_ids ) {
        const auto& token = dgood_table.get( dgood_id, "token does not exist" );
        eosio_assert( token.owner == from, "must be token owner" );

        stats_index stats_table( get_self(), token.category.value );
        const auto& dgood_stats = stats_table.get( token.token_name.value, "dgood stats not found" );

        eosio_assert( dgood_stats.transferable == true, "not transferable");
...
ACTION dgoods::transferft(name from,
    ...
    eosio_assert( dgood_stats.transferable == true, "not transferable");
    eosio_assert( dgood_stats.fungible == true, "Must be fungible token");
...
ACTION dgoods::listsalenft(name seller,
    ...
    eosio_assert( net_sale_amount.amount > .02, "minimum price of at least 0.02 EOS");
    eosio_assert( net_sale_amount.symbol == symbol( symbol_code("EOS"), 4), "only accept EOS for sale" );

    ask_index ask_table( get_self(), get_self().value );
    auto ask = ask_table.find( dgood_id);
    eosio_assert ( ask == ask_table.end(), "already listed for sale" );
    ...
    ask_table.emplace( seller, [&]( auto& ask ) {
        ...
        ask.expiration = time_point_sec(now()) + WEEK_SEC;
    });
}
...
ACTION dgoods::closesalenft(name seller,
    ...
    if ( time_point_sec(now()) > ask.expiration ) {
        ask_table.erase( ask );
        ...
        require_auth( seller );
        eosio_assert( ask.seller == seller, "only the seller can cancel a sale in progress");
...
ACTION dgoods::buynft(name from,
    ...
    eosio_assert ( ask.amount.amount == quantity.amount, "send the correct amount");
    eosio_assert (ask.expiration > time_point_sec(now()), "sale has expired");
...
void dgoods::sub_balance( name owner, uint64_t category_name_id, dasset quantity ) {
    ...
    account_index from_account( get_self(), owner.value );
    const auto& acct = from_account.get( category_name_id, "token does not exist in account" );
    eosio_assert( acct.amount.amount >= quantity.amount, "quantity is more than account balance");
